...
@section('content')
                ...
                @forelse ($reservations as $reservation)
                    <div class="flex justify-center  md:m-6 mb-4 rounded-lg bg-gray-200">
                        ...
                        <div class="m-3 p-1 md:w-[500px]">
                            ...
                            <td class="px-4 py-3 text-sm"> Driver name :
                                @if ($reservation->car->driver_name)
                                    <button class="driver-info-button" 
                                            data-name="{{ $reservation->car->driver_name }}"
                                            data-phone="{{ $reservation->car->driver_phone }}"
                                            data-nid="{{ $reservation->car->driver_nid }}"
                                            data-photo="{{ $reservation->car->driver_photo }}">
                                        {{ $reservation->car->driver_name }}
                                    </button>
                                @else
                                    No Driver
                                @endif
    ...
    <script>
        ...
        // Function to display driver information in a pop-up window
    function showDriverInfo(name, phone, nid, photo) {
        // Create the content for the pop-up window
        var content = "<p><strong>Name:</strong> " + name + "</p>" +
                      "<p><strong>Phone:</strong> " + phone + "</p>" +
                      "<p><strong>NID:</strong> " + nid + "</p>" +
                      "<img src='" + photo + "' alt='Driver Photo'>";

        // Create the pop-up window
        var popup = window.open("", "Driver Information", "width=400,height=300");

        // Write the content to the pop-up window
        popup.document.write(content);
    }

    // Add click event listener to the driver name button
    document.addEventListener('DOMContentLoaded', function() {
        var driverInfoButtons = document.querySelectorAll('.driver-info-button');
        driverInfoButtons.forEach(function(button) {
            button.addEventListener('click', function() {
                var name = this.getAttribute('data-name');
                var phone = this.getAttribute('data-phone');
                var nid = this.getAttribute('data-nid');
                var photo = this.getAttribute('data-photo');
                showDriverInfo(name, phone, nid, photo);
            });
        });
    });
    </script>
